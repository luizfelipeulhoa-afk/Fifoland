...
exports.detectTaskDraft = detectTaskDraft;
const INTENT = /\b(preciso|tenho que|tenho de|devo|quero fazer|vou fazer|não posso esquecer|nao posso esquecer|me lembra|me lembre|lembrar de|marca pra mim|anota pra mim)\b/i;
const WEEKDAYS = [
    [/\bdomingo\b/i, 0],
...
function setTime(date, text, fallbackHour) {
...
function parseDueDate(text, reference) {
...
function detectTaskDraft(text, reference = new Date()) {
    if (!INTENT.test(text))
        return null;
