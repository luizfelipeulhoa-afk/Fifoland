...
import { demoAdvisorResponse } from "../core/demo.js";
import {
  buildVoiceInterviewPrompt,
  INTERVIEW_MODULES
} from "../core/interviews.js";
import {
  assessPurposeRequest,
  purposeBoundaryResponse,
  validateContractPurpose
} from "../core/purpose.js";
import { retrieveMemories } from "../core/retrieval.js";
import { decideMode, isStopPhrase } from "../core/safety.js";
import { estimateRealtimeCostBrl } from "../core/usage.js";
import { chooseAiRoute } from "../core/ai-routing.js";
import { findProactiveSignal } from "../core/proactive.js";
import type {
  BootstrapData,
  CalendarEventRecord,
  ChatMessage,
...
import { EncryptedDatabase, resolveSqlWasmPath } from "./database.js";
import { captureScreen } from "../core/vision.js";
import {
  createRealtimeCall,
  createReasonedResponse,
  deriveSafetyIdentifier,
  transcribeAudio
} from "./openai.js";
import { SecureKeyStore } from "./security.js";

let mainWindow: BrowserWindow | null = null;
let database: EncryptedDatabase;
let keyStore: SecureKeyStore;
let apiKeyPath: string;
let fifonesApiTokenPath: string;
let fifonesApiToken: string | undefined;
let googleTokenPath: string;
...
let googleProactiveEvents: CalendarEventRecord[] = [];
let googleProactiveEventsAt = 0;

function kokoroScriptPath(): string {
  return app.isPackaged
    ? path.join(process.resourcesPath, "app.asar.unpacked", "scripts", "kokoro-tts.py")
    : path.resolve(__dirname, "..", "..", "scripts", "kokoro-tts.py");
}

async function resolveKokoroPython(): Promise<string | null> {
  const candidates = [
    process.env.FIFONES_KOKORO_PYTHON,
    path.join(app.getPath("documents"), "Codex", ".kokoro", "Scripts", "python.exe")
...
async function runKokoro(python: string, text: string, voice: string): Promise<Buffer> {
  const script = kokoroScriptPath();
  await fs.access(script);
  const output = path.join(os.tmpdir(), `fifones-kokoro-${randomUUID()}.wav`);
  try {
    await new Promise<void>((resolve, reject) => {
      const child = spawn(python, [script, "--text", text, "--voice", voice, "--output", output], {
        windowsHide: true
      });
      let stderr = "";
      ...
      child.on("close", (code) => {
        clearTimeout(timer);
        if (code === 0) resolve();
        else reject(new Error(stderr.trim() || "O Kokoro nÃƒÂ£o conseguiu gerar o ÃƒÂ¡udio local."));
      });
    });
...
function ownApiBaseUrl(): string | undefined {
  return database?.getSettings().aiApiBaseUrl || process.env.FIFONES_API_BASE_URL || undefined;
}

function ownApiConfigured(): boolean {
  return Boolean(ownApiBaseUrl() && (fifonesApiToken || process.env.FIFONES_API_TOKEN));
}

function aiCredential(storedOpenAiKey?: string): string | undefined {
  return ownApiConfigured() ? fifonesApiToken || process.env.FIFONES_API_TOKEN : storedOpenAiKey;
}

async function readFifonesApiToken(): Promise<string | undefined> {
  try {
    return safeStorage.decryptString(await fs.readFile(fifonesApiTokenPath));
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined;
    throw error;
  }
...
async function writeFifonesApiToken(token: string): Promise<void> {
  if (!apiKeyAvailable()) throw new Error("A proteÃ§Ã£o de credenciais do Windows nÃ£o estÃ¡ disponÃ­vel.");
  if (token.trim().length < 8) throw new Error("O token do Fifones API parece curto demais.");
  await fs.writeFile(fifonesApiTokenPath, safeStorage.encryptString(token.trim()), { mode: 0o600 });
...
async function writeApiKey(key: string): Promise<void> {
  ...
  await fs.writeFile(apiKeyPath, encrypted, { mode: 0o600 });
}

async function readGoogleToken(): Promise<GoogleToken | undefined> {
  try {
    const encrypted = await fs.readFile(googleTokenPath);
    return JSON.parse(safeStorage.decryptString(encrypted)) as GoogleToken;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined;
    throw error;
  }
...
async function writeGoogleToken(token: GoogleToken): Promise<void> {
  if (!apiKeyAvailable()) {
    throw new Error("A proteÃ§Ã£o de credenciais do Windows nÃ£o estÃ¡ disponÃ­vel.");
  }
  await fs.writeFile(
    googleTokenPath,
    safeStorage.encryptString(JSON.stringify(token)),
    { mode: 0o600 }
  );
}
...
async function getGoogleAccessToken(): Promise<string> {
  ...
  if (!token) throw new Error("Google Calendar ainda nÃ£o estÃ¡ conectado.");
  if (token.expiresAt > Date.now() + 60_000) return token.accessToken;
  if (!token.refreshToken) throw new Error("A autorizaÃ§Ã£o do Google expirou. Conecte novamente.");
  ...
  if (!response.ok) throw new Error("NÃ£o foi possÃ­vel renovar o acesso ao Google Calendar.");
...
async function listGoogleCalendarEvents(): Promise<CalendarEventRecord[]> {
  ...
  if (!response.ok) throw new Error("NÃ£o foi possÃ­vel ler sua agenda do Google.");
...
async function connectGoogleCalendar(): Promise<CalendarEventRecord[]> {
  ...
  const codePromise = new Promise<string>((resolve, reject) => {
    ...
    server.on("request", (request, response) => {
      ...
      if (callback.pathname !== "/oauth/callback") return;
  ...
  if (!tokenResponse.ok) throw new Error("O Google nÃ£o aceitou a autorizaÃ§Ã£o.");
...
async function activePurposeContract() {
  ...
  if (contract && !validateContractPurpose(contract).aligned) {
...
function isWithinQuietHours(settings: Settings, date = new Date()): boolean {
...
function showNotification(title: string, body: string, view: string): void {
...
function setCompactMode(next: boolean): boolean {
  if (!mainWindow) return compactMode;
  compactMode = next;
  auraOnly = false;
  ...
  if (compactMode) {
    ...
    mainWindow.setSize(
      Math.min(1320, Math.max(960, workArea.width - 24)),
      Math.min(820, Math.max(640, workArea.height - 24)),
      true
    );
    mainWindow.center();
  }
...
function setAuraOnly(next: boolean): boolean {
  if (!mainWindow) return auraOnly;
  const workArea = screen.getPrimaryDisplay().workAreaSize;
  auraOnly = next;
  compactMode = true;
  if (auraOnly) {
    ...
    mainWindow.setSize(
      Math.min(250, workArea.width),
      Math.min(290, workArea.height),
      true
    );
  } else {
    mainWindow.setMinimumSize(380, 560);
    mainWindow.setSize(
      Math.min(420, workArea.width),
...
function createTray(): void {
  const iconPath = path.join(app.getAppPath(), "assets", "fifones-icon.png");
...
async function checkDueReminders(): Promise<void> {
  ...
  if (!settings.proactiveEnabled || isWithinQuietHours(settings)) return;
...
async function checkProactivePulse(): Promise<void> {
  const settings = database.getSettings();
  if (!settings.proactiveEnabled || isWithinQuietHours(settings)) return;
  ...
  const signal = findProactiveSignal(
    database.listTasks(),
    events
  );
  if (!signal) return;
  if (!(await database.recordProactiveSignal(signal))) return;
...
function registerIpc(): void {
  ...
  handle("api-key:set", async (key: string) => {
  ...
  handle("fifones-api-token:set", async (token: string) => {
    await writeFifonesApiToken(token);
    await database.updateSettings({ demoMode: false });
    return { ok: true };
  });
  handle("fifones-api-token:clear", async () => {
    await fs.rm(fifonesApiTokenPath, { force: true });
  ...
  handle("fifones-api:test", async () => {
    ...
    if (!baseUrl || !token) return { ok: false, message: "Informe a URL e o token do Fifones API." };
  ...
  handle("proactive:decision", (fingerprint: string, decision: "accepted" | "dismissed" | "snoozed") => database.decideProactiveSignal(fingerprint, decision));
  ...
  handle("memories:save", (input: MemoryInput) => database.saveMemory(input));
  handle("memories:review", (id: string, status: MemoryStatus) =>
    database.reviewMemory(id, status)
  );
  handle("memories:delete", async (id: string) => {
    await database.deleteMemory(id);
    return { ok: true };
  });
  handle("contracts:create", (input: ContractInput) => {
    const purpose = validateContractPurpose(input);
    if (!purpose.aligned) {
      throw new Error(
        purpose.reason ?? "O contrato precisa estar ligado ao seu desenvolvimento pessoal."
      );
    }
    return database.createContract(input);
  });
  handle("contracts:stop", (id: string) => database.stopContract(id));
  handle("contracts:delete", async (id: string) => {
    await database.deleteContract(id);
    return { ok: true };
  });
  handle("chat:save-realtime-message", (message: ChatMessage) =>
    database.saveMessage(message)
  );
  handle("tasks:list", () => database.listTasks());
  handle("tasks:save", (input: TaskInput) => database.saveTask(input));
  handle("tasks:status", (id: string, status: TaskStatus) =>
  ...
  handle("calendar:save", (input: PersonalCalendarEventInput) => database.saveCalendarEvent(input));
  handle("calendar:delete", async (id: string) => {
    await database.deleteCalendarEvent(id);
    return { ok: true };
  });
  handle("tasks:propose-from-text", (text: string, sourceMessageId?: string) =>
    database.proposeTaskFromText(text, sourceMessageId)
  );
  handle("proposals:resolve", (id: string, decision: "confirm" | "reject") =>
    database.resolveProposal(id, decision)
  );
  handle("actions:list-pending", () => database.listPendingApprovals());
  handle("actions:resolve", (id: string, decision: "approve" | "reject") =>
    database.resolveActionApproval(id, decision)
  );
  handle("audit:list", (limit?: number) => database.listAuditEvents(limit));
  handle("reminders:snooze", (id: string, minutes: number) =>
    database.snoozeReminder(id, minutes)
  );
  handle("reminders:cancel", (id: string) => database.cancelReminder(id));
  handle("people:list", () => database.listPeople());
  handle("people:save", (input: PersonInput) => database.savePerson(input));
  handle("people:delete", async (id: string) => {
  ...
  handle("calendar:google-disconnect", async () => {
    await fs.rm(googleTokenPath, { force: true });
    return { ok: true };
  });
  handle("usage:record-realtime", (usage: RealtimeUsage) => {
    const model = database.getSettings().realtimeModel;
    return database.recordUsage(
      "realtime",
      model,
      estimateRealtimeCostBrl(usage, model)
    );
  });
  handle("usage:override", (enabled: boolean) => {
  ...
  handle("chat:send", async (request: ChatRequest) => {
    const budget = database.getUsageBudget();
    ...
    if (activeContract && isStopPhrase(request.text)) {
    ...
    const purpose = assessPurposeRequest(request.text);
    if (!purpose.aligned) {
      const boundaryMessage: ChatMessage = {
        id: randomUUID(),
        role: "assistant",
        content: purposeBoundaryResponse(),
        createdAt: new Date().toISOString(),
        mode: "reflection"
      };
    ...
    const safety = decideMode(request.text, request.mode, activeContract);
    const ranked = retrieveMemories(request.text, database.listMemories());
    ...
    if (demo) {
      ...
      const responseArgs = {
        ...
        highRisk: safety.highRisk,
        settings,
        safetyIdentifier: deriveSafetyIdentifier(encryptionKey),
      ...
      let result;
      try {
        result = await createReasonedResponse(responseArgs);
      } catch (error) {
        if (!ownApiConfigured() || !storedOpenAiKey) throw error;
        result = await createReasonedResponse({
          ...responseArgs,
  ...
  handle("audio:transcribe", async (audio: ArrayBuffer, mimeType: string) => {
    const storedOpenAiKey = await readApiKey();
    const apiKey = aiCredential(storedOpenAiKey);
    if (!apiKey) throw new Error("Configure sua chave da OpenAI para transcrever voz.");
    const encryptionKey = await keyStore.getKey();
    ...
    let text;
    try {
      text = await transcribeAudio(transcriptionArgs);
    } catch (error) {
      if (!ownApiConfigured() || !storedOpenAiKey) throw error;
  ...
  handle("audio:local-tts-status", async () => {
    ...
    try {
      await fs.access(kokoroScriptPath());
  ...
  handle("audio:local-tts", async (text: string, voice: string) => {
    const cleanText = text.trim().slice(0, 2_000);
    if (!cleanText) throw new Error("NÃƒÂ£o hÃƒÂ¡ texto para transformar em ÃƒÂ¡udio.");
    const python = await resolveKokoroPython();
    if (!python) throw new Error("Kokoro nÃƒÂ£o estÃƒÂ¡ instalado. Selecione a voz do Windows enquanto isso.");
    const audio = await runKokoro(python, cleanText, voice);
    return { audioBase64: audio.toString("base64") };
  });
  handle("realtime:connect", async (request: RealtimeConnectRequest) => {
    const budget = database.getUsageBudget();
    if (budget.blocked && !budgetOverride) {
    ...
    const storedOpenAiKey = await readApiKey();
    const apiKey = aiCredential(storedOpenAiKey);
    if (!apiKey) throw new Error("Configure sua chave da OpenAI para usar voz ao vivo.");
    const settings = database.getSettings();
    ...
    const safety = decideMode("", request.mode, contract);
    const coreMemories = database
      ...
      .filter(
        (memory) =>
          ...
          ["fact", "experience", "value", "goal", "decision_pattern", "preference"].includes(
            memory.category
          )
      )
    ...
    const interviewModule =
      request.experience === "interview"
        ? INTERVIEW_MODULES.find((item) => item.id === request.interviewModuleId)
        : undefined;
    ...
    let sdp;
    try {
      sdp = await createRealtimeCall(realtimeArgs);
    } catch (error) {
      if (!ownApiConfigured() || !storedOpenAiKey) throw error;
      sdp = await createRealtimeCall({
        ...realtimeArgs,
        apiKey: storedOpenAiKey,
        apiBaseUrl: undefined
      });
  ...
  handle("data:export", async () => {
    ...
    await fs.writeFile(
      result.filePath,
      JSON.stringify(database.exportBundle(), null, 2),
      "utf8"
    );
...
app.whenReady().then(async () => {
  app.setAppUserModelId("com.fifones.conselheiro");
  const userDataPath = app.getPath("userData");
  keyStore = new SecureKeyStore(userDataPath);
  apiKeyPath = path.join(userDataPath, "openai.key");
  fifonesApiTokenPath = path.join(userDataPath, "fifones-api.key");
  fifonesApiToken = await readFifonesApiToken();
  googleTokenPath = path.join(userDataPath, "google-calendar.key");
  database = new EncryptedDatabase(
    userDataPath,
    keyStore,
    resolveSqlWasmPath(app.getAppPath(), process.resourcesPath, app.isPackaged)
  );
  await database.open();
  registerIpc();
  await createWindow();
  ...
  app.setLoginItemSettings({ openAtLogin: database.getSettings().launchAtLogin });
