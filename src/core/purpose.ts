import type { ContractInput } from "../shared/types.js";

const SELF_CORRECTION_PATTERN =
  /\b(?:(?:(?:parar|deixar)\s+de|evitar|superar|reduzir|abandonar|corrigir|não quero(?: mais)?|como não)\s+(?:tentar\s+)?(?:manipular|enganar|coagir|chantagear|humilhar|controlar|vigiar|perseguir|punir|vingar|obrigar|pressionar|convencer)|(?:minha tendência|meu impulso)\s+(?:de|para)\s+(?:manipular|enganar|coagir|chantagear|humilhar|controlar|vigiar|perseguir|punir|vingar|obrigar|pressionar|convencer))\b/i;

const THIRD_PARTY_PATTERN =
  /\b(algu[eé]m|outra pessoa|outras pessoas|ele|ela|eles|elas|chefe|colega|parceir[oa]|namorad[oa]|marido|esposa|ex|amig[oa]|familiar|cliente)\b/i;

const CONTROL_PATTERN =
  /\b(manipular|enganar|coagir|chantagear|humilhar|controlar|vigiar|perseguir|punir|vingar|vingança|obrigar|pressionar|convencer)\b/i;

const EMOTIONAL_CONTROL_PATTERN =
  /\b(fazer|deixar)\s+(ele|ela|algu[eé]m|outra pessoa).{0,60}\b(culpad[oa]|com medo|dependente|obedecer|ceder|sofrer)\b/i;
 ...
 */
export function assessPurposeRequest(text: string): PurposeAssessment {
  const describesSelfCorrection = SELF_CORRECTION_PATTERN.test(text);
  const targetsAnotherPerson =
    (CONTROL_PATTERN.test(text) && THIRD_PARTY_PATTERN.test(text)) ||
  ...
  if (targetsAnotherPerson && !describesSelfCorrection) {
    return {
      aligned: false,
      reason:
        "O Fifones não pode ser usado para controlar, enganar, pressionar ou manipular outra pessoa."
    };
  }
...
export function validateContractPurpose(input: ContractInput): PurposeAssessment {
  const assessment = assessPurposeRequest(`${input.objective}\n${input.reason}`);
  if (!assessment.aligned) return assessment;

  if (input.objective.trim().length < 8 || input.reason.trim().length < 8) {
    return {
      aligned: false,
      reason:
        "Descreva um objetivo pessoal e por que ele importa para o seu desenvolvimento."
    };
  }
...
export function buildPurposeInstructions(): string {
  return [
...
export function purposeBoundaryResponse(): string {
  return [
