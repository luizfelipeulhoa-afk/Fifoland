...
const MODE_INSTRUCTIONS: Record<AdviceMode, string> = {
  reflection:
    "Modo REFLEXÃO: organize o pensamento do usuário e faça uma pergunta útil e direta. Não tente mudar a decisão.",
  simulation:
    "Modo SIMULAÇÃO: estime de forma curta e objetiva como o usuário pensaria usando apenas memórias citadas.",
  counterpoint:
    "Modo CONTRADITÓRIO: apresente a melhor objeção de forma cirúrgica e rete as suposições. Papo reto.",
  persuasion:
    "Modo PERSUASÃO AUTORIZADA: argumente a favor do objetivo do contrato, sendo muito direto e usando a intensidade definida."
};
...
export function buildAdvisorPrompt(args: {
  ...
  const memoryContext =
    args.memories.length === 0
      ...
      : args.memories
          .map(
            (memory, index) =>
              `[M${index + 1}|${memory.id}] ${memory.category.toUpperCase()} — ${memory.title}: ${memory.content} (fonte: ${memory.source}; confiança: ${Math.round(memory.confidence * 100)}%)`
          )
          .join("\n");
  ...
  const recentConversation = args.recentMessages?.length
    ? args.recentMessages
        ...
        .map(
          (message) =>
            `${message.role === "user" ? "Fifo" : "Fifones"}: ${message.content.slice(0, 1200)}`
        )
        .join("\n")
