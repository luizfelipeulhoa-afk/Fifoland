import type { PersonalCalendarEvent, TaskRecord, VoiceLanguage } from "../shared/types";
...
export function resolveLocalVoiceCommand(
  rawText: string,
  ...
  if (/\b(minhas tarefas|tarefas de hoje|pending tasks|my tasks|tasks today)\b/i.test(text)) {
    ...
    if (!pending.length) return english ? "You have no pending tasks." : "Voce nao tem tarefas pendentes.";
  ...
  if (/\b(proximo evento|minha agenda|next event|my schedule)\b/i.test(text)) {
    ...
    if (!next) return english ? "There are no upcoming local events." : "Nao ha eventos locais proximos.";
