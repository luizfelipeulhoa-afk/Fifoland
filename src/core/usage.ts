import type { RealtimeUsage } from "../shared/types";

const USD_TO_BRL_ESTIMATE = 5.5;

export function estimateRealtimeCostBrl(
  usage: RealtimeUsage,
  model: string
): number {
  ...
  const usd =
    ((usage.inputTextTokens ?? 0) * textInput +
...
export function estimateReasoningCostBrl(
  inputTokens: number,
  outputTokens: number,
  model: string,
  cachedInputTokens = 0
  ...
  return (
    ((((input - cached) * rates.input + cached * cachedRate) * (longContext ? 2 : 1) +
